...
ENGINEERING_DATA_SYSTEM = (
    ...
    "- A PIPE SPECIFICATIONS section with pipe spec strings read directly from the drawing\n"
    "- A VALVE AND FITTING SURVEY section with every valve and fitting catalogued from the drawing\n\n"
    "RULES:\n"
    "- Use the PIPE SPECIFICATIONS section as the authoritative source for pipe spec strings.\n"
    "  Do NOT normalise or correct them -- if the drawing says DB9, report DB9, not 089.\n"
    "- Use the VALVE AND FITTING SURVEY section as the primary source for valve inventory.\n"
    "  Consolidate duplicate entries from multiple tiles into a single clean inventory.\n"
    "- If a VALVE DETAIL ZOOM section is present, it is a closer, higher-magnification "
    "re-examination of specific items the survey pass itself flagged as ambiguous. Where it "
    "covers the same reference number or location as the general survey, its type "
    "determination OVERRIDES the survey entry -- use the zoom section's answer and its 'Why' "
    "reasoning, not the survey's original guess. If the zoom section reports "
    "CANDIDATES: A or B for an item, list it as uncertain between those two types rather than "
    "picking one.\n"
...
LINE_LIST_SYSTEM = (
    ...
    "IMPORTANT: The description contains a PIPE SPECIFICATIONS section with pipe specs read directly\n"
    "from the drawing. Use ONLY those specs as your source -- do not substitute or normalise characters.\n"
    "If the spec reads DB9, report DB9. If it reads 089, report 089. Report exactly what was read.\n\n"
    "RULES:\n"
